...
function main() {
  ...
  function updateCellGeometry(x, y, z) {
    ...
    if (!mesh) {
      ...
      /* added */
      // webSocket.send(JSON.stringify({type: "say", data: x+y+z}));
    }
  ...
  axios
  .get('http://localhost:28080/v1/points')
  .then(res =>{
    res.data.forEach( point =>{
      world.setVoxel(point.x, point.y, point.z, point.voxelId);
    })
  }).catch(e =>{
    console.log(e)
  })
  ...
  function placeVoxel(event) {
    ...
    if (intersection) {
      ...
      if(voxelId === 0){
        sendMessage('delete', point)
        
      } else {
        sendMessage('put', point)
      }
...
function sendMessage(type, data) {
  ...
  //↓worldがMainで定義されているため、ここを解決する必要がある
function receievMsg(msg) {
  if (msg.msgType == "put") {
      ...
      world.setVoxel(...pos, msg.data.voxelId);
      console.log('pos: '+ pos)
      updateVoxelGeometry(...pos);
      requestRenderIfNotRequested();
  }
  if (msg.msgType == "delete") {
    console.log("delete");
    console.log(msg.data)
    const pos= []
    pos.push(msg.data.x)
    ...
    world.setVoxel(...pos, msg.data.voxelId);
    console.log('voxelId: '+ msg.data.vexelId)
    updateVoxelGeometry(...pos);
    requestRenderIfNotRequested();
}
}
